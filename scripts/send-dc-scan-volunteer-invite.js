#!/usr/bin/env node
/**
 * Email DC Scan volunteers the dashboard link + instructions.
 *
...
async function main() {
  const to = VOLUNTEERS.map((v) => v.email);
  const cc = [DEFAULT_SUPERVISOR_EMAIL];
  ...
  const result = await notifyVolunteerInvite(resend, { cc });
  if (result?.error) {
